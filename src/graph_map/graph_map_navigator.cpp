...
bool GraphMapNavigator::AutomaticMapInterchange(Affine3d &Tnow, const Matrix6d &cov, Affine3d & T_world_to_local_map,bool &changed_map_node,bool &created_map_node){
  ...
  if(! currentNode_->WithinRadius(Tnow,interchange_radius_)){ //No longer within radius of node
    ...
    if( changed_map_node=SwitchToClosestMapNode(Tnow,cov,T_world_to_local_map,interchange_radius_)){
